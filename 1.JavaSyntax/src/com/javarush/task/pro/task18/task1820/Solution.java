...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        ...
        Optional maxPrice = Optional.of(cars.max((x1, x2) -> x1.getPrice() - x2.getPrice())).get();
        return maxPrice;
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        //напишите тут ваш код
        Optional maxPrice = Optional.of(cars.filter(x-> x.getPrice() > mostExpensiveCar.getPrice()).findFirst()).get();
        return maxPrice;
    }
}
